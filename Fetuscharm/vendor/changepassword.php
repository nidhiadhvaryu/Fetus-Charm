...
if (isset($_POST['submit']) == "submit") {
    ...
    $currpw = $_POST['cpw'];
    $rnewpw = $_POST['rnpw'];
    $data = $link->rawQueryOne("select * from vendorregistrationtb where emailID=?", array($username));
    if ($link->count == 0) {
        echo "Data Not Found";
    } else {
        $data = $link->rawQueryOne("select password from vendorregistrationtb where emailID=?", array($username));
        if ($data['password'] != $currpw) {

            header('location:changepassword.php?err=1');
        } else {
            $link->where("emailID", $username);
            $sql = $link->update("vendorregistrationtb", array("password" => $rnewpw));
            header("location:index.php");
        }
    }
}
...
<head>
    ...
    <title>Fetus Charm Vendor - Password Change </title>
